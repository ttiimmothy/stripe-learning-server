...
export class UserService {
  ...
  async findAll(): Promise<User[]> {
    try {
      return await this.userRepository.find({}, 'id email role', {
        sort: { createdAt: -1 },
      });
    } catch (error) {
  ...
  async updateUserRole({
    userId,
    role,
  }: UpdateUserRoleInput): Promise<{ message: string; user: UserType }> {
    try {
  ...
  async editProfile(editProfileInput: EditProfileInput) {
    try {
      const { userId, username, profilePicture, bio, profession } =
        editProfileInput;
      if (!userId) {
