...
export class ProductService {
  ...
  async createProduct(
    ...
    try {
      const savedProduct =
        await this.productRepository.create(createProductInput);
  ...
  async getProducts(getProductsInput: GetProductsInput): Promise<{
  ...
  async getProductsSearch(
    ...
    try {
      const { searchQuery, page = '1', limit = '16' } = getProductsSearchInput;
      const filter = {
        $or: [
  ...
  async getProductById(
    productId: string,
  ): Promise<{ product: ProductDocument; reviews: ReviewDocument[] }> {
    try {
