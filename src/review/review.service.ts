...
export class ReviewService{
  constructor(private readonly reviewRepository: ReviewRepository, private readonly productRepository: ProductRepository){}
  ...
  async createReview(createReviewInput: CreateReviewInput): Promise<{message: string, reviews: Review[]}>{
    try {
      const {comment, rating, productId, userId} = createReviewInput;
      if (!comment || !rating || !userId || !productId) {
        throw new BadRequestException("All fields are required");
      }
      const existingReview = await this.reviewRepository.findOne({productId, userId});
      if (existingReview) {
        existingReview.comment = comment;
        existingReview.rating = rating;
        await this.reviewRepository.findByIdAndUpdate(existingReview._id, existingReview);
      } else {
        await this.reviewRepository.create({comment, rating, productId, userId});
      }
  ...
  async reviewsByUserId(userId: string): Promise<Review[]>{
    try {
      ...
      const reviews = await this.reviewRepository.find({userId}, {sort: {createdAt: -1}, populate:{path: "userId", select: "email username role"}});
      if (reviews.length === 0) {
      ...
      throw new InternalServerErrorException("Failed to fetch reviews by user id");
    }
  }
  ...
  async reviewsByProductId(productId: string): Promise<Review[]>{
    try {
      ...
      const reviews = await this.reviewRepository.find({productId}, {sort: {createdAt: -1}, populate:{path: "userId", select: "email username role"}});
      console.log(reviews);
      if (reviews.length === 0) {
      ...
      throw new InternalServerErrorException("Failed to fetch reviews by product id");
    }
  }
}